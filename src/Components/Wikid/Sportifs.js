...
const Sportifs = () => {
      ...
      const fetchData = async () => {
          try {
              ...
              const filteredApiData = data.data
                  ?.filter(post => post.attributes?.Choose === 'Les sportifs')
                  .map(post => {
  ...
  const staticItems = i18n.language === 'fr' ? Sportifsitems : Sportifsitemsar;

  // Combine static and API data
  const combinedItems = [...staticItems, ...apiItems];

  
// Sort combined items by publishedAt date in ascending order (earliest to latest)
